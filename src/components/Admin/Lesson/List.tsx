...
import { deleteLesson, getLessonList } from '@/hooks/useEvents';
...
const LessonList = ({ url }: Props) => {
  ...
  const lessonDelete = async () => {
    ...
    await deleteLesson(String(userToken), Number(selectedElements));
          ...
          <tbody>
            {lessonList?.map((item, index) => (
              <tr key={index}>
                ...
                <td className="border-b  border-[#eee] px-4 py-4  dark:border-strokedark ">
                  <Link
                    href={`/admin/lesson/users/exam/${item?.id}`}
                    className="inline-flex items-center w-[130px] justify-center rounded-md border border-primary bg-primary/5 px-5 py-1 text-center text-[15px] font-medium text-primary hover:bg-opacity-90"
                  >
                    Харах
                  </Link>
                </td>
